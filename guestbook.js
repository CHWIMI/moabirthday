...
onAuthStateChanged(auth, (user) => {
    ...
    // 마감 후 등장하는 관리자 전용 텍스트 링크 버튼 상태 업데이트
    const adminFallbackBtn = document.getElementById('admin-fallback-btn');
    if (adminFallbackBtn) {
        adminFallbackBtn.textContent = (user && !user.isAnonymous) ? '관리자 로그아웃' : '관리자 접속';
    }
...
if (form) {
    if (isGuestbookClosed) {
        // 마감 후: 폼 전체를 안내 메시지로 교체하고 조그만 관리자 접속 링크 추가
        form.innerHTML = `
            <div style="text-align:center; padding: 1.5rem 0 0.5rem 0;">
                <p class="notice-text" style="margin-bottom: 1rem;">방명록 작성 기간이 종료되었습니다. 감사합니다! 🎂</p>
                <div style="text-align: right; width: 100%;">
                    <span id="admin-fallback-btn" style="color: var(--text-light); opacity: 0.6; font-size: 0.75rem; text-decoration: underline; cursor: pointer; transition: opacity 0.2s;">관리자 접속</span>
                </div>
            </div>
        `;
