...
import { getAuth, signInAnonymously, onAuthStateChanged, GoogleAuthProvider, signInWithPopup } from "https://www.gstatic.com/firebasejs/10.9.0/firebase-auth.js";
...
const ADMIN_UIDS = ["GOOGLE_UID_1", "GOOGLE_UID_2"]; // 관리자 구글 UID를 여기에 입력하세요
...
onAuthStateChanged(auth, (user) => {
    ...
    if (googleLoginBtn && user) {
        if (!user.isAnonymous) {
            googleLoginBtn.innerHTML = '<img src="https://www.gstatic.com/firebasejs/ui/2.0.0/images/auth/google.svg" alt="Google" style="width: 20px; height: 20px;">구글 로그인됨';
            googleLoginBtn.style.backgroundColor = '#f1f8ff';
            googleLoginBtn.style.color = '#0366d6';
            googleLoginBtn.style.borderColor = '#c8e1ff';
                ...
                if (entryDiv) {
                    ...
                    if (data.isBlinded) {
                        if (isAdmin()) {
                            ...
                            displayMessage = `🚨 관리자에 의해 블라인드 처리된 게시글입니다.`;
                            opacityStyle = 'color: var(--text-light); font-style: italic;';
                        }
                    }

                    entryDiv.querySelector('.guestbook-entry-name').innerHTML = escapeHtml(data.name || '익명');
                    entryDiv.querySelector('.guestbook-entry-message').innerHTML = displayMessage;
                    entryDiv.querySelector('.guestbook-entry-message').style.cssText = opacityStyle;
                    entryDiv.querySelector('.guestbook-entry-header').style.cssText = opacityStyle;
                    entryDiv.querySelector('.guestbook-entry-date').innerText = formatDate(createdAt);
                }
            }
