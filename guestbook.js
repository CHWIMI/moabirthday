...
import {
    ...
    collection, addDoc, onSnapshot, query, orderBy, limit, serverTimestamp, doc, deleteDoc, updateDoc, getDoc
...
// 2. 로컬 캐시를 활성화하여 db 초기화 (1번 최적화 적용)
...
// --- 방명록 공지 박스 관련 로직 (C안 변형: 10초 타이머) ---
let noticeTimer = null;
const noticeBox = document.getElementById('gb-focus-notice');
const closeNoticeBtn = document.getElementById('close-gb-notice');

function showNotice() {
    if (!noticeBox) return;

    // 이미 열려있다면 기존 타이머만 초기화하고 다시 10초 카운트
    if (noticeTimer) {
...
// 입력창 포커스 시 공지 노출
if (nameInput) nameInput.addEventListener('focus', showNotice);
if (messageInput) messageInput.addEventListener('focus', showNotice);
...
let currentLimit = 10; // 2번 최적화: 초기 로드 10개로 제한
...
onAuthStateChanged(auth, (user) => {
    ...
    if (googleLoginBtn && user) {
        if (!user.isAnonymous) {
            googleLoginBtn.innerHTML = '<img src="https://www.gstatic.com/firebasejs/ui/2.0.0/images/auth/google.svg" alt="Google">구글 로그아웃';
    ...
    // 마감 후 등장하는 로그인 텍스트 링크 버튼 상태 업데이트
    const adminFallbackBtn = document.getElementById('admin-fallback-btn');
    if (adminFallbackBtn) {
        adminFallbackBtn.textContent = (user && !user.isAnonymous) ? '구글 로그아웃' : '구글 로그인';
    }
...
// 방명록 엔트리 HTML 생성 공통 함수
// added/modified 양쪽에서 호출되어 중복 코드를 제거합니다.
function buildEntryHTML(docId, data, createdAt, isOwner) {
    let actionButtons = '';
    ...
    if (data.isBlinded) {
        // 블라인드 처리 로직
        if (isAdmin()) {
            // 관리자에게는 희미하게 원본 노출 + 복구/삭제 버튼
            displayMessage = `[블라인드 됨]<br>${displayMessage}`;
            opacityStyle = 'style="opacity: 0.5;"';
            actionButtons = `
                <div class="guestbook-entry-actions">
                    <button class="guestbook-action-btn unblind-btn" data-id="${docId}" style="color: #28a745; border-color: #28a745;">🩹 복구하기</button>
                    <button class="guestbook-action-btn delete-btn" data-id="${docId}">삭제</button>
                </div>
            `;
        } else {
        ...
        // 블라인드 되지 않은 일반 글 — 관리자/소유자 분기 (if-else if 체인)
        if (isAdmin() && isOwner) {
            // 관리자 본인 글: 수정 + 숨기기 + 삭제
            actionButtons = `
                <div class="guestbook-entry-actions">
                    <button class="guestbook-action-btn edit-btn" data-id="${docId}">수정</button>
                    <button class="guestbook-action-btn blind-btn" data-id="${docId}" style="color: #dc3545; border-color: #dc3545;">🚨 숨기기</button>
                    <button class="guestbook-action-btn delete-btn" data-id="${docId}">삭제</button>
                </div>
            `;
        } else if (isAdmin()) {
...
function loadGuestbook() {
    ...
    unsubscribe = onSnapshot(q, (snapshot) => {
        ...
        // --- DocumentFragment 적용 시작 ---
        const fragment = document.createDocumentFragment();
        const isInitialLoad = listContainer.children.length === 0;
        ...
        snapshot.docChanges().forEach((change) => {
            ...
            if (change.type === "added") {
                ...
                entryDiv.innerHTML = buildEntryHTML(change.doc.id, data, createdAt, isOwner);

                if (isInitialLoad) {
                    // 초기 로드 시에는 Fragment에 모아둠
                    fragment.appendChild(entryDiv);
                } else {
                    // 실시간 추가(예: 새 글 작성) 시에는 기존 로직대로 위치에 맞게 DOM에 직접 삽입
        ...
        // 초기 로드였다면 모아둔 Fragment를 한 번에 DOM에 삽입하여 렌더링 최적화
        if (isInitialLoad) {
            listContainer.appendChild(fragment);
        }
        // --- DocumentFragment 적용 끝 ---
...
if (form) {
    if (isGuestbookClosed) {
        // 마감 후: 폼 전체를 안내 메시지로 교체하고 조그만 로그인 링크 추가
        form.innerHTML = `
            <div style="text-align:center; padding: 2rem 0 1rem 0;">
                <p class="notice-text" style="text-align: center; font-size: 1.3rem; font-weight: 600; color: var(--text-color); margin-bottom: 1.5rem;">
                    방명록 작성 기간이 종료되었습니다.<br>감사합니다! 🎂
                </p>
                <div style="text-align: center; width: 100%;">
                    <span id="admin-fallback-btn" style="color: var(--text-light); opacity: 0.6; font-size: 0.75rem; text-decoration: underline; cursor: pointer; transition: opacity 0.2s;">구글 로그인</span>
                </div>
            </div>
        `;

        const adminBtn = document.getElementById('admin-fallback-btn');
        if (adminBtn) {
            // 호버 효과
            adminBtn.addEventListener('mouseenter', () => adminBtn.style.opacity = '1');
            adminBtn.addEventListener('mouseleave', () => adminBtn.style.opacity = '0.6');
            ...
            adminBtn.addEventListener('click', async () => {
                ...
                try {
                    ...
                    guestbookLoaded = false;
                    listContainer.innerHTML = '';
                    if (isGuestbookVisible) { guestbookLoaded = true; loadGuestbook(); }
                } catch (error) {
                    console.error("구글 로그인 실패:", error);
                }
            });
